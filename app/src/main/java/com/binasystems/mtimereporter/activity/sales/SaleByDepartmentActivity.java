...
import com.binasystems.mtimereporter.fragment.sales.SalesByDepartmentFragment;

import java.util.Date;

public class SaleByDepartmentActivity extends BaseActivity implements View.OnClickListener{
	TextView cancel;

	public static void startAcitivty(Context caller, Date date, ComaxApiManager.DateViewType dateViewType){
		Intent intent = new Intent(caller, SaleByDepartmentActivity.class);
		if(date != null){
			intent.putExtra(SalesByDepartmentFragment.EXTRA_DATE, date);
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(savedInstanceState == null){
			Fragment fragment = new SalesByDepartmentFragment();
